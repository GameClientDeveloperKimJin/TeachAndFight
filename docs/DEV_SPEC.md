...
| `self_wall_dist` | number | 0~6 (m, 등 뒤 벽까지 거리) |
| `self_action_duration` | number | 0~60 (초, 현재 self_action 유지시간) |
| `enemy_action_duration` | number | 0~60 (초, 현재 enemy_action 유지시간) |
| `enemy_whiff_count` | number | 0~20 (최근 5초 내 상대 헛스윙 횟수) |

> `self_wall_dist`~`enemy_whiff_count` 4종은 #26(어휘 확장 Tier1)에서 추가. 신규 애니메이션 불필요 — 기존 시뮬레이션 상태 노출만.
...
| `idle` | — | 대기 (스태미나 회복 가속) |
| `counter_attack` | — | 발동 0.08s(light보다 빠름)·사거리 1.5(heavy와 동일)·스태미나 15. 상대 startup 중 커밋 시 데미지 1.5배(8→12). 애니는 Attack_Light 재사용. #26 Tier2, 밸런스 패치로 전용 수치 분리 |
| `feint` | — | 데미지 없는 페이크. light_startup으로 위장, 빠른 recovery. #26 Tier2 |
...
9. needs_confirmation=false(=규칙이 실제로 적용됨)일 때 disciple_reply는 **절대 물음표로 끝내지 않는다**. 물음표로 끝나는 문장은 4번 규칙대로 needs_confirmation=true 전용이다 — 적용됐을 땐 "~하겠습니다!", "~하도록 배웠습니다" 처럼 완료형/평서문으로 끝내서, 코치가 "규칙이 적용된 건지 아니면 또 되묻는 건지" 헷갈리지 않게 한다. (10번 규칙의 복합 문장 케이스는 예외.)
