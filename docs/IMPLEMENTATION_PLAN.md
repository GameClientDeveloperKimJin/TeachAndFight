...
**#22(캐릭터 비주얼)는 W3→W4 게이트에서 제외함(2026-07-30 결정)** — 캐릭터당 8클립 × 6명 = 48클립 전부 별도 아트로 제작하기로 해서 작업량이 커짐(`docs/CHARACTER_ART_PLAN.md` 참고). 게이트에 걸어두면 아트 제작이 끝날 때까지 W4(#17~#20) 전체가 묶여버리므로, #22는 별도 병렬 트랙으로 빼고 W4는 placeholder(사각형) 비주얼 상태로 먼저 진행한다. 아트가 완성되는 대로 #17(연출 폴리싱)에 순차 반영. start.sh W3 목록에서도 22 제거함(더 이상 자동 게이트 점검 대상 아님) — 단, GitHub 이슈 #22 자체는 열어둔 채로 별도 진행상황 추적.
...
>
> GitHub 이슈 #22로 등록 완료(2026-07-29). ⚠ 2026-07-30: 제작 물량이 커져서(6명 × 8클립, 별도 아트) W3→W4 게이트에서는 제외 — start.sh W3 목록에서도 22 제거함. 이슈 자체는 열어둔 채 병렬로 진행하고, W4(#17 연출 폴리싱)는 placeholder 비주얼로 먼저 진행하다 아트 완성되는 대로 반영.
